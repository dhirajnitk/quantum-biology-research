...
We parameterize a hypothetical information-theoretic constraint using CCO as a candidate acceptor. The spatial gap between the active zone and mitochondrial inner membrane ($\sim 10$--$20$~nm) is in the near-field regime at $\lambda = 327$~nm ($r/\lambda \approx 0.03$), where evanescent coupling rather than free-space photon propagation applies. We apply the far-field cross-section ratio $p = \sigma/A_{\mathrm{target}}$ strictly as an upper-bound benchmark; this is a simplifying approximation because the cross-section concept assumes propagating waves, whereas the relevant distance scale requires a dipole-dipole coupling matrix (see Limitations).
...
\begin{table*}[htbp]
\caption{Spatial gating with CCO target using far-field benchmark $p = 7.84\times10^{-4}$.}
\label{tab:gating}
\centering
\begin{tabular}{cccc}
\toprule
